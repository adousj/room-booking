...
$(function() {
  ...
  table_td_hover_in = function() {
    ...
    if (!($(this).hasClass('chosen') || $(this).hasClass('neighbour'))) {
      ...
      $('#room-table td.table-item[data-id="' + (data_id - mod_num) + '"]:not(.chosen):not(.neighbour)').css('background-color', hover_neighbour_color);
      return $('#room-table td.table-item[data-id="' + (data_id + mod_num) + '"]:not(.chosen):not(.neighbour)').css('background-color', hover_neighbour_color);
    }
  };
  ...
  table_td_hover_out = function() {
    ...
    if (!($(this).hasClass('chosen') || $(this).hasClass('neighbour'))) {
      ...
      $('#room-table td.table-item[data-id="' + (data_id - mod_num) + '"]:not(.chosen):not(.neighbour)').css('background-color', empty_color);
      return $('#room-table td.table-item[data-id="' + (data_id + mod_num) + '"]:not(.chosen):not(.neighbour)').css('background-color', empty_color);
    }
  };

  $('#room-table td.table-item').hover(table_td_hover_in, table_td_hover_out);

  $('#room-table .table-item').bind('click', function(e) {
    ...
    if ($(this).hasClass('chosen') || $(this).hasClass('neighbour')) {
      ...
      room_id = data_id % mod_num;
      min_t = Math.floor(Math.min.apply(Math, [data_id / mod_num, chosen_id / mod_num]));
      max_t = Math.floor(Math.max.apply(Math, [data_id / mod_num + 1, chosen_id / mod_num + 1]));
      ...
      $('#room-table td.table-item[data-id="' + (data_id - mod_num) + '"]').css('background-color', neighbour_color).addClass('neighbour');
      return $('#room-table td.table-item[data-id="' + (data_id + mod_num) + '"]').css('background-color', neighbour_color).addClass('neighbour');
    }
  });
